...
def make_evoked_from_raw(raw, epoch_len=None):
    sf = float(raw.info['sfreq'])
    dur = float(raw.n_times) / sf

    # If user supplies a value, clamp it. Otherwise use (almost) the full recording.
    if epoch_len is not None:
        epoch_len = max(0.10, min(epoch_len, max(0.10, dur - 1.0 / sf)))
    else:
        epoch_len = max(1.0, dur - 1.0 / sf)

    # One long event (no overlap) so the viewer spans the whole window
    events = mne.make_fixed_length_events(raw, duration=epoch_len, overlap=0.0, start=0.0, stop=None)

    # tmax must be strictly less than epoch duration
    tmax = epoch_len - 1.0 / sf

    picks = mne.pick_types(raw.info, eeg=True, meg=False, stim=False, eog=False, exclude='bads')
    epochs = mne.Epochs(raw, events, tmin=0.0, tmax=tmax,
...
def main():
    ...
    p.add_argument('--epoch-sec', type=float, default=None)  # NEW
    args = p.parse_args()


    
    # Load data (require a real FIF; no synthetic fallback)
    if args.eeg is None:
        raise SystemExit("No --eeg provided. This viewer requires a real EEG FIF. "
                        "Run pacman_2.py first, then pass --eeg <path-to-eeg_overall.fif>.")
    raw = read_raw_auto(args.eeg)
    ...
    # Preprocess
    raw.load_data()
    # Short-signal safe filtering: use IIR when we don't have many samples
    if raw.n_times < 1024:
    ...
    # Evoked + inverse
    evoked, cov = make_evoked_from_raw(raw, epoch_len=args.epoch_sec)  # CHANGED
