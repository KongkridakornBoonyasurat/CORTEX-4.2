...
class PacmanLogger:
    ...
    def log_episode(self, ep, ret, steps, ate, dopamine_trace, d_star=None, turns=0, ep_dir=None):
        ...
        if hasattr(self, "_last_region_activity"):
            R = np.asarray(self._last_region_activity, dtype=np.float32)
            # only save correlation if we have at least 2 finite values
            if R.size >= 2 and np.all(np.isfinite(R)):
                np.save(os.path.join(ep_dir, f"region_corr_ep{ep:03d}.npy"),
                        np.corrcoef(R))
...
class PacmanEnv:
    def __init__(self, H=10, W=10, seed=42, max_steps=None, step_cost=0.0, wrap=True):
        self.H, self.W = H, W
        self.wrap = bool(wrap)   # torus wrap on/off
        self.rng = random.Random(seed)
        self.max_steps = (max_steps if (max_steps and max_steps > 0) else 4 * self.H * self.W)
        self.step_cost = float(step_cost)
        self.t = 0               # steps so far in this episode
        self.prev_dist = None    # distance to goal at previous step
        self._prev_action = None
        self.d0 = None           # optimal steps from start (for logging)
    ...
    def reset(self):
        ...
        if hasattr(self, "_forced_spawn") and self._forced_spawn is not None:
            self.pos = tuple(self._forced_spawn)
            self.spawn_pos = self.pos
        self._place_food()
        # cache optimal steps from start (for logging/evaluation)
        self.d0 = self._torus_L1(self.pos, self.food)
    ...
    def _place_food(self):
        if hasattr(self, "_food_policy") and self._food_policy:
            # use spawn if available (stage wants food relative to birth), else current pos
            ay, ax = (self.spawn_pos if hasattr(self, "spawn_pos") and self.spawn_pos is not None else self.pos)
            # never place on current pos or spawn
            def _ok(y, x):
            ...
            if self._food_policy == "adjacent_right":
                ...
                for dy, dx in [(-1,0),(1,0),(0,-1),(0,1)]:
                    ...
                    if 0 <= y < self.H and 0 <= x < self.W and _ok(y, x):
                        nbr.append((y, x))
                if nbr:
                    self.food = random.choice(nbr); return
        while True:
            fy = self.rng.randrange(0, self.H)
            fx = self.rng.randrange(0, self.W)
            if (fy, fx) != self.pos and (getattr(self, "spawn_pos", None) is None or (fy, fx) != self.spawn_pos):
                self.food = (fy, fx); return
    def _torus_L1(self, a, b):
        """Toroidal L1 distance between grid cells a=(y,x), b=(y,x)."""
        (y1, x1), (y2, x2) = a, b
    ...
    def step(self, action:int):
        ...
        # time advances
        self.t += 1
        # smooth proximity shaping (exponential “smell”)
        d_prev = self._torus_L1(getattr(self, "_prev_pos", self.pos), self.food)
        d_curr = self._torus_L1(self.pos, self.food)
        lam = 3.0
        phi_prev = math.exp(- d_prev / lam)
        phi_curr = math.exp(- d_curr / lam)
        r_smell = 0.20 * (phi_curr - phi_prev)
        ...
        if self.halo_radius and d_curr > 0 and d_curr <= self.halo_radius:    
            ...
            if turn_pen > 0.0 and self._prev_action is not None and action != self._prev_action:
                d_next = self._torus_L1(self.pos, self.food)
                if d_next > d_curr:
                    reward -= float(turn_pen)
            self._prev_action = action

        # terminal trigger only when not already in afterglow
        ate = (self._afterglow == 0) and (self.pos == self.food)
        ...
        return self.render(), reward, self.done, {"ate": ate, "d0": self.d0}
...
class BrainAdapter:
    ...
    def _lfp_scalar_from_region_dict(self, d):
            ...
            # PRIORITY 1: neural_activity (has brain oscillations from cortex_brain.py)
...
# ---------- Runner ----------
def run(args):
    BASE_ROOT = r"C:\Users\User\Desktop\Brain AI\cortex 4.2\cortex 4.2 v42"
    ...
    for ep in range(args.episodes):
        ...
        MIN_STEPS_FOR_EEG = 50  # you can tune this
        dopamine_trace=[]
        ...
        frame = viewer.draw(obs, f"EP{ep} STEP{steps} R{ret:.1f}")
        rec.record(frame)
        ...
        while not done or steps < MIN_STEPS_FOR_EEG:
            ...
            if 'oscillations' in out:
                osc = out['oscillations']
                osc_mod = []
                for i, reg in enumerate(['motor', 'sensory', 'thalamus', 'cerebellum',
                                        'parietal', 'pfc', 'limbic', 'hippocampus', 
                                        'insula', 'basal_ganglia']):
                    if reg in ['motor', 'basal_ganglia'] and 'beta' in osc:
                        osc_mod.append(1.0 + 0.45 * np.sin(float(osc['beta'])))     # ↑ beta
                    elif reg in ['parietal', 'sensory'] and 'alpha' in osc:
                        osc_mod.append(1.0 + 0.50 * np.sin(float(osc['alpha'])))    # ↑ alpha
                    elif reg in ['hippocampus', 'limbic'] and 'theta' in osc:
                        osc_mod.append(1.0 + 0.25 * np.sin(float(osc['theta'])))    # ↓ theta a bit
                    elif reg == 'pfc' and 'gamma' in osc:
                        osc_mod.append(1.0 + 0.20 * np.sin(float(osc['gamma'])))
                    else:
                        osc_mod.append(1.0)
            else:
                osc_mod = [1.0] * 10
            ...
            _ra_np = np.asarray(_ra, dtype=np.float32)
            
            # Apply oscillation modulation
            if len(osc_mod) == len(_ra_np):
                _ra_np = _ra_np * np.array(osc_mod, dtype=np.float32)
            ...
            # Always use real region activity - show true brain state
            R = 10
            use_ra = (_ra_np[:R] if _ra_np.size >= R else np.pad(_ra_np, (0, R - _ra_np.size))).astype(np.float32)
            ...
            try:
                ...
                # If wrapping is enabled and we crossed an edge, split the tween at the boundary
                def tween_segment(y0, x0, y1, x1, k_frames):
                ...
                if env.wrap:
                    ...
                    # detect vertical wrap (eg 0 -> H-1 or H-1 -> 0)
                    if abs(dy) > 1:
                        # going up across top or down across bottom
                        if dy < 0:  # up: e.g. 0 -> H-1
                            # segment 1: oy,ox -> -0.5,ox (to top edge)
                            tween_segment(oy, ox, -0.5, ox, K // 2 + 1)
                            # segment 2: H-0.5,ox -> ny,nx (from bottom edge)
                            tween_segment(H - 0.5, ox, ny, nx, K - (K // 2 + 1))
                        else:        # down: e.g. H-1 -> 0
                            tween_segment(oy, ox, H - 0.5, ox, K // 2 + 1)
                            tween_segment(-0.5, ox, ny, nx, K - (K // 2 + 1))
                    # detect horizontal wrap (eg 0 -> W-1 or W-1 -> 0)
                    elif abs(dx) > 1:
                        if dx < 0:  # left wrap: 0 -> W-1
                            tween_segment(oy, ox, oy, -0.5, K // 2 + 1)
                            tween_segment(oy, W - 0.5, ny, nx, K - (K // 2 + 1))
                        else:        # right wrap: W-1 -> 0
                            tween_segment(oy, ox, oy, W - 0.5, K // 2 + 1)
                            tween_segment(oy, -0.5, ny, nx, K - (K // 2 + 1))
                    else:
                        tween_segment(oy, ox, ny, nx, K)
                else:
                    # no wrap case: simple straight tween
        ...
        if args.auto_curriculum:
            ...
            # Define d* for this episode (optimal steps from start to food on torus)
...
def generate_thesis_report(bme_collector: BMEDataCollector, output_dir: str):
    ...
    if not all_stats:
        print("[BME] No statistics to report")
        return
    
    # === 1. Learning Curves ===
    # Build these lists FIRST with safe .get() so they exist even if plotting fails
    td_errors   = [ep.get('learning', {}).get('mean_td_error', 0.0) for ep in all_stats]
    values      = [ep.get('learning', {}).get('final_value', 0.0) for ep in all_stats]
    entropies   = [ep.get('learning', {}).get('mean_entropy', 0.0) for ep in all_stats]
    firing_rates= [ep.get('neural_dynamics', {}).get('mean_firing_rate', 0.0) for ep in all_stats]
    ...
    try:
        ...
        da = [ep['neuromodulation']['mean_dopamine'] for ep in all_stats]
        ach = [ep['neuromodulation']['mean_acetylcholine'] for ep in all_stats]
        ne = [ep['neuromodulation']['mean_norepinephrine'] for ep in all_stats]
    ...
    try:
        summary = {
            ...
            'final_td_error': float(td_errors[-1]) if td_errors else 0,
            'final_value': float(values[-1]) if values else 0,
            'mean_firing_rate': float(np.mean(firing_rates)) if firing_rates else 0,
            'mean_dopamine': float(np.mean(da)) if da else 0,
            'total_spikes': sum([ep['neural_dynamics']['spike_count'] for ep in all_stats])
        }
...
def main():
    ...
    ap.add_argument("--no-wrap", action="store_true", help="disable torus wrap; hitting an edge clamps position (no teleport)")
