...
os.environ["QT_OPENGL"] = "software"         # if this still fails, keep "software"; don't use "angle" here
os.environ["PYVISTA_OFF_SCREEN"] = "false"
os.environ["PYVISTA_TRAME_SERVER"] = "off"
...
def make_evoked_from_raw(raw, epoch_len=1.0, step=0.25):
    """Create an Evoked-like object that covers the *entire recording* by
    concatenating overlapping window means (so the time viewer spans full duration)."""
    ...
    s = max(1, int(step * sf))
    if w <= 1 or w > n:
        w = max(2, min(n, int(1.0 * sf)))
    idxs = list(range(0, max(1, n - w + 1), s))
    X = []
    for i in idxs:
        seg = raw.get_data(picks=picks, start=i, stop=i + w)  # shape (ch, w)
        X.append(seg.mean(axis=1, keepdims=True))             # (ch, 1)
    X = np.concatenate(X, axis=1)                             # (ch, n_windows)
    info = mne.pick_info(raw.info, picks)
    evoked = mne.EvokedArray(X, info, tmin=0.0, comment='sliding-avg')
    # time axis = window centers spaced by `step`
    evoked.times = np.arange(X.shape[1]) * step
    # Use the first 10 s (or full file if shorter) for noise covariance
    end_cov = min(10.0, n / sf)
    noise_cov = mne.compute_raw_covariance(raw.copy().crop(0, end_cov),
                                           picks=picks, rank='info')
...
def main():
    ...
    # Preprocess
    raw.load_data()
    # clean mains hum for more human-like spectra
    raw.notch_filter([50, 60], method='iir', iir_params=dict(order=4, ftype='butter'))
    raw.filter(1, 40)
    ...
    # Evoked + inverse
    evoked, cov = make_evoked_from_raw(raw, epoch_len=1.0, step=0.25)
    ...
    # Interactive dynamic source estimate across the full recording
    brain = stc.plot(
        subject='fsaverage', hemi='both', surface='inflated',
        subjects_dir=subjects_dir, time_viewer=True, show_traces=True,
        cortex='low_contrast'
    )
    print('[ok] Interactive viewer opened. Press "h" for shortcuts.')
